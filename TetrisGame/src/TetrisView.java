...
//initTileSize()에서 구한 tileSize를 기반으로 height, gameWidth를 다시 계산하고, gameWidth가 변경될 // 경우 width도 다시 구함
	...
	private void fillSidePanel(Graphics2D g2d) {
		g2d.setPaint(new Color(0, 8, 52)); // 군청색 g2d.fillRect(width - sideWidth, 0, sideWidth, height);
	}
		...
		for (int row = 2; row < gridHeight; row++) {
			for (int col = 0; col < gridWidth; col++) {
				if (grid[row][col] != null) {
					...
					g2d.setPaint(col % 2 == 0 ? Color.BLACK : Color.darkGray);
				}
